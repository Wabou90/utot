...
class Search:
    def create(prompt: str, actualSearch: bool = True, language: str = 'en') -> dict:  # None = no search
        ...
        if cf_clearance == '':
            raise ValueError(
                'cf_clearance must be set, refer to documentation')
...
class Completion:
    def create(
        ...
        completion = ''
        response = post('https://www.phind.com/api/infer/answer', headers=headers,
                        json=json_data, timeout=99999, impersonate='chrome110')
